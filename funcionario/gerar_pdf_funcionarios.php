...
class PDF extends FPDF {
    function Header() {
        ...
        $this->Cell(0, 10, clean_text('Relatório de Funcionários'), 0, 1, 'C');
        $this->Ln(10);
    }
    function Footer() {
        $this->SetY(-15);
...
// Obter os filtros da URL (enviados pela página de funcionários)
$search = isset($_GET['search']) ? trim($_GET['search']) : '';
$colunas_selecionadas = isset($_GET['colunas']) 
    ? (is_array($_GET['colunas']) ? $_GET['colunas'] : explode(',', $_GET['colunas'])) 
    : ['nome', 'email', 'localidade', 'telefone1', 'telefone2'];
$colunas_permitidas = ['id', 'nome', 'email', 'morada', 'localidade', 'telefone1', 'telefone2'];
$colunas_selecionadas = array_intersect($colunas_selecionadas, $colunas_permitidas);

if (empty($colunas_selecionadas)) {
    $colunas_selecionadas = ['nome', 'email', 'localidade', 'telefone1', 'telefone2'];
}

// Construir a mesma query que usa na página de funcionários para obter os dados filtrados
...
foreach($colunas_selecionadas as $coluna) {
    switch($coluna) {
        ...
        case 'email':
            ...
            break;
        case 'morada':
            $header[] = clean_text('Morada');
            $w[] = 60;
            break;
        case 'localidade':
            $header[] = clean_text('Localidade');
            $w[] = 40;
            break;
        case 'telefone1':
            $header[] = clean_text('Telefone 1');
            $w[] = 30;
            break;
        case 'telefone2':
            $header[] = clean_text('Telefone 2');
            $w[] = 30;
            break;
        default:
...
foreach($funcionarios as $funcionario) {
    foreach($colunas_selecionadas as $index => $coluna) {
        $pdf->Cell($w[$index], 6, clean_text($funcionario[$coluna] ?? ''), 1, 0, '', false);
    }
    $pdf->Ln();
}

$conn->close();
$pdf->Output('I', 'relatorio_funcionarios.pdf');
exit(); 